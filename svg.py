...
class Svg:
    ...
    def __init__(self, edge_kerf, spacer_kerf):
        self.kerf = edge_kerf
    ...
    def generate_slotted_top_edge(self, slots, spacer_width, content_width, corner_toothing, edge_width, depth):
        ...
        for idx, slot in enumerate(slots):
            ...
            if idx < len(slots)-1:
                ...
                path_parts.append('h {}'.format(self.k_corr))
                path_parts.append('v {}'.format(INDENT_DEPTH - self.k_corr))
    ...
    def generate_edge(self, slots, spacer_width, content_width, corner_toothing, edge_width, depth):
        path_parts = list()
        # Top edge
        path_parts.append(self.kerf_correct_corner(0))
        path_parts.extend(self.generate_slotted_top_edge(slots, spacer_width, content_width, corner_toothing, edge_width, depth))

        # Right edge
        path_parts.append(self.kerf_correct_corner(1))
        path_parts.extend(self.generate_toothing(1, not corner_toothing, depth, edge_width))
        ...
        path_parts.extend(self.generate_toothing(2, False, content_width, edge_width))
        ...
        # Left edge
        path_parts.append('v -{}'.format(edge_width))
        path_parts.extend(self.generate_toothing(3, not corner_toothing, depth, edge_width))
    ...
    def generate_edges(self, trayspec):
        ...
        paths = list()
        paths.append( self.generate_edge(
        ...
        if horiz_slot_width_sum + spacer_w * (len(horiz_slots_and_widths)-1) < content_width:
            ...
            empty_space = content_width - horiz_slot_width_sum - spacer_w*len(horiz_slots_and_widths)
    ...
    def generate_floor(self, trayspec):
        edge_w = trayspec['edge_width']
        spacer_w = trayspec['spacer_width']
        ...
        p = list()
        p.append(self.kerf_correct_corner(0))
        p.extend(self.generate_toothing(0, True, width, edge_w))
        p.append(self.kerf_correct_corner(1))
        p.extend(self.generate_toothing(1, True, height, edge_w))
        p.append(self.kerf_correct_corner(2))
        p.extend(self.generate_toothing(2, True, width, edge_w))
        p.append(self.kerf_correct_corner(3))
        p.extend(self.generate_toothing(3, True, height, edge_w))
        p.append(self.kerf_correct_corner(4))
        p.append('z')
        ...
        def generate_internal_holes():
            ...
            for idx, col_w in enumerate(col_widths[0:-1]):
                ...
                for index, position_length in enumerate(positions):
                    ...
                    parts.append('m {} {}'.format(self.k_corr, self.k_corr))
                    parts.append('h {}'.format(spacer_w - self.k_corr*2))
                    parts.append('v {}'.format(position_length - self.k_corr*2))
                    parts.append('h -{}'.format(spacer_w - self.k_corr*2))
                    parts.append('v -{}'.format(position_length - self.k_corr*2))
                    parts.append('z')
                    hole_svgs.append({
    ...
    def generate_vert_spacer(self, indent_spaces, content_width, depth, edge_width, spacer_width):
        ...
        height_from_teeth = edge_width + self.k_corr
        if len(positions_for_height) == 1:
            height_from_teeth = 0
        spacer_object = {
            'svg': path_parts,
            'width': content_width + 2 * edge_width + 2 * self.k_corr,
            'height': depth + 2 * self.k_corr + height_from_teeth,
            'offset_x': 0,
            'offset_y': 0,
    ...
    def generate_horiz_spacer(self, col_widths, spacer_indents, content_width, spacer_width, l_edge_width, r_edge_width, depth):
        ...
        spacer_object = {
            'svg': path_parts,
            'width': l_edge_width + sum(col_widths) + spacer_width * (len(col_widths) -1) + r_edge_width + 2 * self.k_corr,
            'height': depth + 2 * self.k_corr,
            'offset_x': 0,
            'offset_y': 0,
            'thickness': spacer_width
        }
    ...
    def create_vertical_spacer_combined_slot_list(self, columns, spacer_w):
        spacer_indent_lists = []
    ...
    def generate_horizontal_spacers(self, columns, edge_width, spacer_width, depth):
        ...
        for idx, column in enumerate(columns):
            ...
            for slot_index, slot in enumerate(slots):
                ...
                spacer_objects.append(self.generate_horiz_spacer(
                    col_slots,
                    spacer_indents,
    ...
    def generate_spacers(self, trayspec):
        ...
        spacer_indent_gaps = self.create_vertical_spacer_combined_slot_list(columns, spacer_w)

        paths = []  # List of lists of strings. Each list of strings contains the svg vector path for an object
        for gaps in spacer_indent_gaps:
            paths.append(self.generate_vert_spacer(gaps, content_width, depth, edge_w, spacer_w))

        horiz_paths = self.generate_horizontal_spacers(columns, edge_w, spacer_w, depth)
    ...
    def draw_spacers(self, dwg, trayspec):
        spacer_objects = self.generate_spacers(trayspec)
    ...
    def draw_edges(self, dwg, trayspec):
        object_padding = 1
        ...
        svg_objects = self.generate_edges(trayspec)
        ...
        floor_object = self.generate_floor(trayspec)
